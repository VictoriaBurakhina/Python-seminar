
#Даны два файла,
# в каждом из которых находится запись многочлена.
# Задача - сформировать файл, содержащий сумму многочленов.


def sum_polynomial(polynomial_list ):
    NO_VALUE = ''
    result_list = []
    result_out = NO_VALUE
    number_value = 0
    for i in range(len(polynomial_list)):
        if polynomial_list[i].find('^')>0:
            find_value_list = polynomial_list[i].split("*")
            type_polynomial = find_value_list[1]
            ratio_polynomial = 0
            for j in range(i,len(polynomial_list)):
                if polynomial_list[j].find(type_polynomial)>0:
                    temp_list = polynomial_list[j].split('*')
                    ratio_polynomial += int(temp_list[0])
                    polynomial_list[j] = NO_VALUE
            result_list.append(f'{ratio_polynomial}*{type_polynomial}')
        else:
            if polynomial_list[i].find('=')<0 and polynomial_list[i] != NO_VALUE:
                if polynomial_list[i-1] == '=':
                    number_value -= int(polynomial_list[i])
                else:
                    number_value += int(polynomial_list[i])

    result_list.append(f'{number_value}')

    for i in range(len(result_list)):
        if i == 0:
            result_out += (f'{result_list[i]}')
        else:
            if result_list[i][0] != '-':
                result_out += (f'+{result_list[i]}')
            else:
                result_out += (f'{result_list[i]}')

    result_out = (f'{result_out}=0')
    result_out = result_out.replace('+',' + ').replace('-',' - ').replace('=',' = ')
    return result_out


# Заполняем файл записями
with open("poly.txt", "w") as file:
    file.write('3*x^9 + 3*x^8 - 2*x^6 + 1*x^5 - 3*x^4 - 3*x^2 + 3 + 2*x^2 + 2*x^1 + 2 = 0' + '\n')
    file.write('-3*x^2 - 10 + 3*x^2 + 1*x^1 = -2' + '\n')
    file.write('3*x^9 + 3*x^8 - 2 = 0' + '\n')

# Заполняем файл записями
with open("poly_2.txt", "w") as file:
    file.write('3*x^9 + 3*x^8 - 2*x^6 + 1*x^5 - 3*x^4 - 3*x^2 + 3 + 2*x^2 + 2*x^1 + 25 = 5' + '\n')
    file.write('3*x^9 + 3*x^8 + 2 = -1' + '\n')
    file.write('3 + 2*x^2 + 2*x^1 = 0' + '\n')


# читаем все строки
with open("poly.txt", "r") as file_poly:
    lines_poly = file_poly.readlines()
    lines_poly = [line for line in lines_poly]

# читаем все строки
with open("poly_2.txt", "r") as file_poly_2:
    lines_poly_2 = file_poly_2.readlines()
    lines_poly_2 = [line for line in lines_poly_2]


# Используем функцию  zip
value_zip = zip(lines_poly, lines_poly_2)
# Используем функцию list comprehension
value_list = [i[0]+i[1] for i in value_zip]
# Используем функцию  map и lambda
value_list = list(map(lambda x: x.replace('+','$+').replace('-','$-').replace('=','$=$').replace(' ', '').replace('\n', '$'), value_list))
# Используем функцию list comprehension
value_list = [i.split('$') for i in value_list]
for i in range(len(value_list)):
    # Используем функцию  filter
    value_list[i] = list(filter(lambda x: x!='', value_list[i]))
result_list = []
for i in range(len(value_list)):
    result_list.append(sum_polynomial(value_list[i]))
# Используем функцию  enumerate
result_enum_list = list(enumerate(result_list))
with open("result_poly.txt", "w") as file:
    file.write('')
for i in result_enum_list:
    print(f'Результат сложения №{i[0]+1} -> {i[1]}')
    with open("result_poly.txt", "a") as file:
        file.write(i[1] + '\n')
    # Используем функцию  zip
list_print = list(zip(lines_poly, lines_poly_2, result_list))
for i in list_print:
    print('')
    print_str = f'poly.txt         -> {i[0]}poly_2.txt       -> {i[1]}result_poly.txt  -> {i[2]}'
    print(print_str)
    print('')